...
function roleLabel(role: WorkspaceRole): string {
  return role === "member" ? "Student" : "Teacher";
}
          ...
          {user?.account_type === "teacher" ? (
            <>
              <p className="mb-4 text-[14px] text-muted-foreground">
                You don&apos;t have a group yet. Create one to add students.
              </p>
              <Button
            ...
            <p className="text-[14px] text-muted-foreground">
              You haven&apos;t been added to a class yet. Ask your teacher to invite you.
            </p>
          )}
        </div>
      </>
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div
          className={cn(
            "rounded-[28px] border border-black/5 bg-white p-5 sm:p-7 shadow-soft",
        ...
        >
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {isTeacher && <ScheduleMeetingDialog workspaceId={workspace.id} />}
              {isTeacher && <ConnectWhatsAppDialog workspace={workspace} />}
              {isTeacher && <StartCallDialog workspaceId={workspace.id} />}
            </div>
          </div>
          <div className="flex flex-col">
            {members?.map((m) => (
              ...
              >
                ...
                <span className="shrink-0 rounded-full bg-secondary px-2.5 py-1 text-[11.5px] font-medium text-foreground/80">
                  {roleLabel(m.role)}
                </span>
              </div>
            ))}
        ...
        {isTeacher && (
          <div className="flex flex-col gap-6">
            <div className="rounded-[28px] border border-black/5 bg-white p-5 sm:p-7 shadow-soft">
              <h3 className="mb-4 text-[16px] font-semibold tracking-tight text-foreground">Invite student</h3>
              <form
                onSubmit={(e) => {
                  e.preventDefault();
            ...
            {invitations && invitations.length > 0 && (
              <div className="rounded-[28px] border border-black/5 bg-white p-5 sm:p-7 shadow-soft">
                <h3 className="mb-4 text-[14px] font-semibold tracking-tight text-foreground">
                  Pending invitations
                </h3>
